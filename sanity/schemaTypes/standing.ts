...
export const standing = defineType({
    ...
    fields: [
        ...
        defineField({
            ...
            type: 'array',
            of: [standingRow],
            validation: (Rule) => Rule.custom(async (rows, context) => {
                ...
                const competition = await client.fetch(`*[_type == "competition" && _id == $id][0]`, { id: competitionId })

                if (competition && competition.config?.teamsCount && rows && rows.length > competition.config.teamsCount) {
