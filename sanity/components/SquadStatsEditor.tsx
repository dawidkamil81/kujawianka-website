...
import { Card, Stack, Text, Button, Checkbox, TextInput, useToast, Spinner, Flex, Box, Label, Badge } from '@sanity/ui'
                ...
                <Flex gap={3} align="center">
                    ...
                    <Box style={inputContainerStyle}>
                        ...
                        <TextInput
                            ...
                            onChange={(e) => updateManualEdit(p._id, 'matches', parseInt(e.currentTarget.value) || 0)}
                    ...
                    <Box style={inputContainerStyle}>
                        ...
                        <TextInput
                            ...
                            onChange={(e) => updateManualEdit(p._id, 'goals', parseInt(e.currentTarget.value) || 0)}
                    ...
                    <Box style={inputContainerStyle}>
                        ...
                        <TextInput
                            ...
                            onChange={(e) => updateManualEdit(p._id, 'assists', parseInt(e.currentTarget.value) || 0)}
                            style={{ width: '50px', textAlign: 'center' }}
                        />
                    </Box>
                    ...
                    <Flex gap={1} style={{ borderLeft: '1px solid #eee', paddingLeft: '10px' }}>
                        <Box style={inputContainerStyle}>
                            ...
                            <TextInput fontSize={1} padding={2} type="number" value={getStatValue(p._id, 'yellowCards')} onChange={(e) => updateManualEdit(p._id, 'yellowCards', parseInt(e.currentTarget.value) || 0)} style={{ width: '40px', textAlign: 'center' }} />
                        </Box>
                        <Box style={inputContainerStyle}>
                            <Label size={0} muted>Czerw.</Label>
                            <TextInput fontSize={1} padding={2} type="number" value={getStatValue(p._id, 'redCards')} onChange={(e) => updateManualEdit(p._id, 'redCards', parseInt(e.currentTarget.value) || 0)} style={{ width: '40px', textAlign: 'center' }} />
                        </Box>
                        {isGK && (
                            <Box style={inputContainerStyle}>
                                <Label size={0} muted>Czyste</Label>
                                <TextInput fontSize={1} padding={2} type="number" value={getStatValue(p._id, 'cleanSheets')} onChange={(e) => updateManualEdit(p._id, 'cleanSheets', parseInt(e.currentTarget.value) || 0)} style={{ width: '40px', textAlign: 'center' }} />
                            </Box>
                        )}
                    </Flex>
                </Flex>
...
export function SquadStatsEditor(props: any) {
    ...
    const updateMatchStat = useCallback((id: string, field: string, value: any) => {
        setMatchStats(prev => ({ ...prev, [id]: { ...prev[id], [field]: value } }))
    }, [])

    const updateManualEdit = useCallback((id: string, field: string, value: number) => {
        setManualEdits(prev => ({ ...prev, [id]: { ...prev[id], [field]: value } }))
    }, [])

    const getStatValue = useCallback((id: string, field: string) => {
            ...
            <Stack space={5}>
                ...
                {allPlayers.filter(p => !POSITIONS_ORDER.includes(p.position)).length > 0 && (
                    <Box>
                        ...
                        <Stack space={0}>
                            {allPlayers.filter(p => !POSITIONS_ORDER.includes(p.position)).map(p => (
                                <EditPlayerRow
                                    key={`edit-rest-${p._id}`}
                                    p={p}
                                    squadIds={squadIds}
